...
be included (if present) if a time value is given."
  (CL:LET* ((BUFFER (MAKE-RAW-MUTABLE-STRING 25)) (INDEX 0))
   ...
   (CL:WHEN (CL:NOT (CL:= (%DATE-TIME-YEAR DATE) NULL-INTEGER))
    (CL:SETQ INDEX
     (INSERT-STRING
      (INTEGER-TO-STRING (CL:TRUNCATE (%DATE-TIME-YEAR DATE))) 0
     ...
     (CL:WHEN (CL:NOT (CL:= (%DATE-TIME-DAY DATE) NULL-INTEGER))
      ...
      (CL:WHEN (CL:NOT (CL:= (%DATE-TIME-HOUR DATE) NULL-INTEGER))
       ...
       (CL:SETQ INDEX
        (INSERT-STRING
         (FORMAT-WITH-PADDING
          (INTEGER-TO-STRING (CL:TRUNCATE (%DATE-TIME-HOUR DATE))) 2
       ...
       (CL:WHEN (CL:NOT (CL:= (%DATE-TIME-MINUTE DATE) NULL-INTEGER))
        ...
        (CL:SETQ INDEX
         (INSERT-STRING
          (FORMAT-WITH-PADDING
           (INTEGER-TO-STRING (CL:TRUNCATE (%DATE-TIME-MINUTE DATE))) 2
           #\0 KWD-DATE-TIME-RIGHT CL:NIL)
          0 NULL-INTEGER BUFFER INDEX KWD-DATE-TIME-PRESERVE))
        (CL:WHEN (CL:NOT (CL:= (%DATE-TIME-SECOND DATE) NULL-INTEGER))
         ...
         (CL:SETQ INDEX
          (INSERT-STRING
           (FORMAT-WITH-PADDING
            (INTEGER-TO-STRING (CL:TRUNCATE (%DATE-TIME-SECOND DATE)))
            2 #\0 KWD-DATE-TIME-RIGHT CL:NIL)
           0 NULL-INTEGER BUFFER INDEX KWD-DATE-TIME-PRESERVE))))
       (CL:WHEN (CL:NOT (CL:= (%DATE-TIME-ZONE DATE) NULL-FLOAT))
        (CL:IF (CL:= (%DATE-TIME-ZONE DATE) 0.0d0)
         ...
         (CL:SETQ INDEX
          (INSERT-STRING
           (TIME-ZONE-FORMAT60 (%DATE-TIME-ZONE DATE) CL:T) 0
           NULL-INTEGER BUFFER INDEX KWD-DATE-TIME-PRESERVE))))))))
...
(CL:DEFMETHOD HELP-MERGE-NULL-FIELDS ((SELF DECODED-DATE-TIME) DEFAULT INDEX)
  ...
  (CL:WHEN
   (CL:AND (CL:= (%DATE-TIME-YEAR SELF) NULL-INTEGER) (CL:>= INDEX 0))
  ...
  (CL:WHEN
   (CL:AND (CL:= (%DATE-TIME-DAY SELF) NULL-INTEGER) (CL:>= INDEX 2))
   (CL:SETF (%DATE-TIME-DAY SELF) (%DATE-TIME-DAY DEFAULT)))
  (CL:WHEN
   (CL:AND (CL:= (%DATE-TIME-HOUR SELF) NULL-INTEGER) (CL:>= INDEX 3))
   (CL:SETF (%DATE-TIME-HOUR SELF) (%DATE-TIME-HOUR DEFAULT)))
  (CL:WHEN
   (CL:AND (CL:= (%DATE-TIME-MINUTE SELF) NULL-INTEGER)
    (CL:>= INDEX 4))
   (CL:SETF (%DATE-TIME-MINUTE SELF) (%DATE-TIME-MINUTE DEFAULT)))
  (CL:WHEN
   (CL:AND (CL:= (%DATE-TIME-SECOND SELF) NULL-INTEGER)
    (CL:>= INDEX 5))
   (CL:SETF (%DATE-TIME-SECOND SELF) (%DATE-TIME-SECOND DEFAULT)))
  (CL:WHEN
   (CL:AND (CL:= (%DATE-TIME-MILLIS SELF) NULL-FLOAT) (CL:>= INDEX 6))
...
(CL:DEFMETHOD ENCODE-CALENDAR-DATE ((TIME-STRUCTURE DECODED-DATE-TIME))
  ...
  (CL:RETURN-FROM ENCODE-CALENDAR-DATE
   (MAKE-DATE-TIME
    (CL:IF
     (CL:NOT (CL:= (%DATE-TIME-YEAR TIME-STRUCTURE) NULL-INTEGER))
     (%DATE-TIME-YEAR TIME-STRUCTURE)
     (%DATE-TIME-YEAR *DEFAULT-DECODED-DATE-TIME*))
    (CL:IF
     (CL:NOT (CL:= (%DATE-TIME-MONTH TIME-STRUCTURE) NULL-INTEGER))
  ...
  (CL:LET* ((TZ (GET-LOCAL-TIME-ZONE)))
   ...
   (CL:LET*
    ...
    (INSERT-AT SUBSTITUTION-LIST (WRAP-STRING "MONTH")
     (CL:AREF
      (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *MONTH-NAME-VECTOR*))
      MONTH))
    ...
    (INSERT-AT SUBSTITUTION-LIST (WRAP-STRING "DOW")
     (WRAP-STRING
      (STRING-CAPITALIZE (SUBSEQUENCE (%SYMBOL-NAME DOW) 0 3))))
    (INSERT-AT SUBSTITUTION-LIST (WRAP-STRING "DATE")
     (WRAP-STRING
      (CONCATENATE
       (FORMAT-WITH-PADDING (INTEGER-TO-STRING (CL:TRUNCATE DAY)) 2 #\0
...
(CL:DEFUN STARTUP-DATE-TIME ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:LET*
     ((I NULL-INTEGER) (ITER-035 0) (UPPER-BOUND-036 12)
      (ABBREVIATION NULL)
      (ITER-037
       (GET-QUOTED-TREE
     ...
     (CL:LOOP WHILE
      (CL:AND (CL:<= ITER-035 UPPER-BOUND-036)
       (CL:NOT (CL:EQ ITER-037 NIL)))
    ...
    (CL:LET*
     ((I NULL-INTEGER) (ITER-039 0) (UPPER-BOUND-040 12)
      (ABBREVIATION NULL)
      (ITER-041
       (GET-QUOTED-TREE
     ...
     (CL:LOOP WHILE
      (CL:AND (CL:<= ITER-039 UPPER-BOUND-040)
       (CL:NOT (CL:EQ ITER-041 NIL)))
