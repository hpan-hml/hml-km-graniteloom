...
(CL:DEFUN DECODE-XML-ENTITY-REF (DOCTYPE NAME PE-REFERENCE-ALLOWED?)
  ...
  #+MCL
  (CL:CHECK-TYPE NAME CL:SIMPLE-STRING)
  (CL:LET* ((VALUE STELLA::NULL-STRING))
   ...
   (CL:WHEN (CL:EQ VALUE STELLA::NULL-STRING)
    (CL:SETQ VALUE
     (%WRAPPER-VALUE (LOOKUP *XML-BASE-ENTITY-TABLE* (WRAP-STRING NAME)))))
...
(CL:DEFUN DECODE-XML-STRING (DOCTYPE INPUT PE-REFERENCE-ALLOWED?)
  ...
  (CL:LET*
   ...
   (CL:IF (NULL? REFERENCE-START) (CL:RETURN-FROM DECODE-XML-STRING INPUT)
    (CL:LET* ((BUFFER (NEW-OUTPUT-STRING-STREAM)) (REFERENCE-END 0))
     ...
     (CL:LOOP WHILE (DEFINED? REFERENCE-START) DO
      ...
      (CL:WHEN (CL:NOT (DEFINED? REFERENCE-END))
        ...
        (CL:T (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "<" TAGNAME)
         ...
         (CL:IF IN-CONTENT-FIELD?
          (CL:PROGN
           ...
           (CL:WHEN (CL:AND (DEFINED? INDENT) (CL:> INDENT 0))
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) EOL)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "/>" EOL))))))
     (CL:T
      (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
