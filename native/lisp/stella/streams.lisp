...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-FLOAT NULL-CHARACTER *SAFETY*
...
(CL:DEFUN SET-LOGGING-PARAMETERS (MODULE CL:&REST |PARAMS&VALUES|)
  ...
  (CL:CHECK-TYPE MODULE CL:SIMPLE-STRING)
  (CL:LET* ((ARGLIST-000 NIL))
   (CL:LET* ((ARG-000 NULL) (ITER-000 |PARAMS&VALUES|) (COLLECT-000 NULL))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG-000 (CL:POP ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS ARG-000 NIL))
       (CL:IF (CL:EQ ARGLIST-000 NIL) (CL:SETQ ARGLIST-000 COLLECT-000)
   ...
   (CL:LET*
    ((OPTIONS
     ...
     (LOGINFO NULL))
    (CL:LET*
     ((MOD NULL) (PROPS NULL) (ITER-001 (%THE-KV-LIST *LOGGING-REGISTRY*)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NULL)) DO
      (CL:SETQ MOD (%KEY ITER-001)) (CL:SETQ PROPS (%VALUE ITER-001))
      (CL:WHEN (STRING-EQUAL? MODULE (%WRAPPER-VALUE MOD))
    ...
    (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-002 (%THE-PLIST OPTIONS)))
...
log level of `module'.  Return FALSE if any of them are undefined."
  ...
  (CL:LET*
   ...
   (CL:RETURN-FROM LOG-LEVEL<=
    (CL:AND (DEFINED? POS1) (DEFINED? POS2) (CL:<= POS1 POS2)))))
...
or :EOL to print a line terminator."
  ...
  (CL:WHEN (LOG-LEVEL<= LOGLEVEL MODULE)
   (CL:LET*
    ...
    (CL:SETQ ROOM
     (CL:- ROOM (CL:+ 27 (CL:THE CL:FIXNUM (CL:LENGTH MODULEPREFIX)))))
    (CL:LET* ((*PRINTREADABLY?* CL:T))
     (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (CL:LET* ((ELT NULL) (ITER-000 MESSAGE))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
           ...
           (CL:PROGN
            (CL:COND
             ((STRING-EQL? (%SYMBOL-NAME ELT) "EOL")
              (%%PRINT-STREAM (%NATIVE-STREAM LOG) EOL) (CL:SETQ ROOM MAXROOM)
              (CL:GO :CONTINUE))
...
value is defined.\" :PUBLIC? TRUE)" (CL:FUNCTION LOOKUP-LOGGING-PARAMETER)
  ...
  :MAX-WIDTH  - logging output lines will be kept to approximately this width
                (defaults to 10000, minimum width of about 30 is used to
                print line header information).\" :PUBLIC? TRUE)"
    (CL:FUNCTION SET-LOGGING-PARAMETERS) NULL))
  :VOID)
