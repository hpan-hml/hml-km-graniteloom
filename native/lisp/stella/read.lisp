   ...
   (CL:LET*
    ...
    (CL:LET*
     ...
     (CL:COND
      ...
      ((CL:EQ TOK_STELLALOGICALSTATENAME_ KWD-READ-ERROR)
       (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-003) "Illegal read syntax: `"
         (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_ TOK_TOKENSTART_ TOK_CURSOR_
          TOK_SIZE_ CL:NIL)
         "'")
      ...
      (CL:T
       (CL:LET* ((STREAM-004 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-004) "Illegal symbol syntax: `"
         (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_ TOK_TOKENSTART_ TOK_CURSOR_
          TOK_SIZE_ CL:NIL)
         "' in state `" (CL:AREF TOK_STATENAMES_ TOK_STATE_) "'")
        (CL:ERROR (NEW-READ-EXCEPTION (THE-STRING-READER STREAM-004))))))
     ...
     (CL:IF
      ...
      (CL:LET* ((STREAM-005 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-005) "Garbage found after `"
        (%SYMBOL-NAME RESULT) "'")
       (CL:ERROR (NEW-READ-EXCEPTION (THE-STRING-READER STREAM-005)))))))))
...
(CL:DEFMETHOD STREAM-TO-STRING ((FROM INPUT-STREAM))
  ...
  (CL:LET*
   ((TO (NEW-OUTPUT-STRING-STREAM))
    (BUFFER (CL:MAKE-STRING *TOKENIZER-INITIAL-BUFFER-SIZE*)) (BYTES-READ 0))
   (CL:DECLARE (CL:TYPE CL:FIXNUM BYTES-READ))
   (CL:LOOP
    (CL:SETQ BYTES-READ
     (BYTE-ARRAY-READ-SEQUENCE BUFFER FROM 0 *TOKENIZER-INITIAL-BUFFER-SIZE*))
...
(CL:DEFUN HELP-STARTUP-READ5 ()
  (CL:PROGN
   ...
   (CL:LET* ((SELF-056 (NEW-PROPERTY-LIST)))
    (CL:SETF (%THE-PLIST SELF-056)
    ...
    (CL:SETQ *STELLA-LOGICAL-STATE-NAMES-TABLE* SELF-056))
   (CL:SETQ *STELLA-LOGICAL-STATE-NAMES*
    (NEW-VECTOR (LENGTH (%STATE-NAMES *STELLA-TOKENIZER-TABLE*))))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-058 0)
     (UPPER-BOUND-059 (CL:1- (LENGTH *STELLA-LOGICAL-STATE-NAMES*))))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-058 UPPER-BOUND-059))
    (CL:LOOP WHILE (CL:<= ITER-058 UPPER-BOUND-059) DO (CL:SETQ I ITER-058)
     (CL:LET
     ...
     (CL:WHEN
      ...
      (CL:LET
       ...
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))
     (CL:SETQ ITER-058 (CL:1+ ITER-058))))
