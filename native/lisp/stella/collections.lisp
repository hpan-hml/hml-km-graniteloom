...
(CL:DEFMETHOD REVERSE ((SELF VECTOR-SEQUENCE))
  ...
  (CL:LET*
   ((ARRAY (%THE-ARRAY SELF)) (BOTTOM 0)
    (TOP (CL:1- (%SEQUENCE-LENGTH SELF))) (ELT NULL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR ARRAY)
   ...
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y)
    ...
    (CL:PROGN
     (CL:WHEN (CL:= (%SEQUENCE-LENGTH X) (%SEQUENCE-LENGTH Y))
      (CL:LET*
       ((ELTX NULL) (VECTOR-000 X) (INDEX-000 0)
        (LENGTH-000 (%SEQUENCE-LENGTH VECTOR-000)) (ELTY NULL)
        (VECTOR-001 Y) (INDEX-001 0)
...
(CL:DEFCLASS HEAP (VECTOR)
  ((PREDICATE :ALLOCATION :INSTANCE :ACCESSOR %PREDICATE)
   (FILL-POINTER :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION
    :INSTANCE :ACCESSOR %FILL-POINTER))
  (:DOCUMENTATION
...
(CL:DEFUN NEW-HEAP (PREDICATE ARRAY-SIZE)
  ...
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE HEAP)))
...
(CL:DEFUN ACCESS-HEAP-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-FILL-POINTER)
    (CL:IF SETVALUE?
     (CL:SETF (%FILL-POINTER SELF) (%WRAPPER-VALUE VALUE))
...
the same."
  (CL:LET*
   ((VALUES (%THE-ARRAY SELF)) (CURSOR (%FILL-POINTER SELF))
    (VALUE NULL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES)
    (CL:TYPE CL:FIXNUM CURSOR))
   (CL:COND
    ((CL:EQ PREDICATE NULL) (CL:SETQ PREDICATE (%PREDICATE SELF)))
...
(CL:DEFUN NEW-STELLA-HASH-TABLE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%EQUAL-TEST? SELF) CL:NIL)
...
(CL:DEFUN REHASH-STELLA-HASH-TABLE (SELF NEWSIZE)
  ...
  (CL:WHEN (NULL-ARRAY? (%THE-TABLE SELF))
...
(CL:DEFUN NEW-1D-ARRAY (DIM1)
  ...
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE 1D-ARRAY)))
...
(CL:DEFUN NEW-2D-ARRAY (DIM2 DIM1)
  ...
  (CL:CHECK-TYPE DIM1 CL:FIXNUM)
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE 2D-ARRAY)))
